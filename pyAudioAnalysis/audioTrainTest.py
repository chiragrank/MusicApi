...
def featureAndTrainRegression(dirName, mtWin, mtStep, stWin, stStep, modelType, modelName, computeBEAT=False):
    ...
    '''
    # STEP A: Feature Extraction:
    [features, _, fileNames] = dirsWavFeatureExtraction([dirName], mtWin, mtStep, stWin, stStep, computeBEAT=computeBEAT)
    features = features[0]
...
def fileClassification(inputFile, modelName, modelType):
    ...
    [Fs, x] = readAudioFile(inputFile)        # read audio file and convert to mono
    x = stereo2mono(x)
    ...
    # feature extraction:
    [MidTermFeatures, s] = mtFeatureExtraction(x, Fs, mtWin * Fs, mtStep * Fs, round(Fs * stWin), round(Fs * stStep))
    MidTermFeatures = MidTermFeatures.mean(axis=1)        # long term averaging of mid-term statistics
    if computeBEAT:
        [beat, beatConf] = beatExtraction(s, stStep)
